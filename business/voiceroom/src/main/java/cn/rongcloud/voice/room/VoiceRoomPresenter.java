...
//    private List<Shield> shields = new ArrayList<>();
    ...
    private void joinRoom(String roomId, boolean isCreate) {
        ...
        if (isCreate) {
            ...
            RCVoiceRoomEngine.getInstance().createAndJoinRoom(roomId, rcVoiceRoomInfo, new RCVoiceRoomCallback() {
                @Override
                public void onSuccess() {
                    LogCat.d("==============createAndJoinRoom onSuccess");
                ...
                @Override
                public void onError(int code, String message) {
                    LogCat.e("==============createAndJoinRoom onError,code:" + code + ",message:" + message);
                    mView.dismissLoading();
                    KToast.show("创建房间失败");
                    closeRoom();
                }
            ...
            RCVoiceRoomEngine.getInstance().joinRoom(roomId, new RCVoiceRoomCallback() {
                ...
                @Override
                public void onError(int code, String message) {
                    LogCat.e("==============joinRoom onError,code:" + code + ",message:" + message);
                    mView.dismissLoading();
                }
            });
        }
    ...
    private void sendSystemMessage() {
        if (mVoiceRoomBean != null) {
            ...
            tips.setContent("感谢使用随心语音 语音房，请遵守相关法规，不要传播低俗、暴力等不良信息。欢迎您把使用过程中的感受反馈给我们。");
...
//        });
    ...
    public void sendGift() {
        ...
        LogCat.e(TAG, "uiSeatModels:" + uiSeatModels.size());
        for (UiSeatModel uiSeatModel : uiSeatModels) {
            LogCat.e(TAG, "uiSeatModel:" + GsonUtil.obj2Json(uiSeatModel.getMember()));
...
//            }
        ...
        LogCat.e(TAG, "getCreateUserId:" + getCreateUserId());
        LogCat.e(TAG, "memberArrayList:" + GsonUtil.obj2Json(memberArrayList));
    ...
    public void clickSendGift(User user) {
        ...
        Member member = Member.fromUser(user);
        if (!TextUtils.isEmpty(userId)) {
            LogCat.e(TAG, "clickSendGift: userId = " + userId);
            ArrayList<UiSeatModel> uiSeatModels = voiceRoomModel.getUiSeatModels();
                ...
                if (result.ok()) {
                    ...
                    if (roomBean != null) {
                        ...
                        if (roomBean.isPrivate()) {
                            inputPasswordDialog = new InputPasswordDialog(((VoiceRoomFragment) mView)
