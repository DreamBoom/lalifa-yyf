...
fun throttleClick(wait: Long = 500, block: ((View) -> Unit)): View.OnClickListener {
...
fun debounceClick(wait: Long = 500, block: ((View) -> Unit)): View.OnClickListener {
    return View.OnClickListener { v ->
...
fun Array<View>.onClick(wait: Long = 500, block: ((View) -> Unit)) {
    forEach {
        it.setOnClickListener(throttleClick(wait, block))
    }
}

fun View.onDebounceClick(wait: Long = 500, block: ((View) -> Unit)) {
    setOnClickListener(debounceClick(wait, block))
}

fun View.floatInput(root:View){
    val softKeyboardStateListener: SoftKeyBroadManager.SoftKeyboardStateListener =
        object : SoftKeyBroadManager.SoftKeyboardStateListener {
    ...
    val softKeyBroadManager = SoftKeyBroadManager(root)
