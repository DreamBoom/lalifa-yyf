...
fun Long.formatByte(): String {
    ...
    return when {
        this >= GB_2_BYTE -> df.format(this.toDouble() / GB_2_BYTE).replace() + "GB"
        this >= MB_2_BYTE -> df.format(this.toDouble() / MB_2_BYTE).replace() + "MB"
        this >= KB_2_BYTE -> df.format(this.toDouble() / KB_2_BYTE).replace() + "KB"
